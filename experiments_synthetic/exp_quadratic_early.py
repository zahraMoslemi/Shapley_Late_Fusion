...
if True:
    ...
    noisy_dim = int(sys.argv[1])
    noise_ratio = float(sys.argv[2])
    seed = int(sys.argv[3])


# Fixed exp parameters
repetition=1

# Data model parameters
n = 2000
dim_modalities = [noisy_dim, 100]
dim_latent = [20, 20, 0]
noise_ratios = [noise_ratio, 0.1]
trans_type = ["quadratic", "linear", "linear"]
mod_prop = [1, 1, 0, 1]
interactive_prop = 0.1

mod_outs = [[0, 50, 60, 70, 80], [0, 60, 70, 80, 90]]
num_modalities = 2
...
ckpt_dir = f"./checkpoints/{exp_name}/noisy_dim{noisy_dim}_noise_ratio{noise_ratio}_seed{seed}/"
...
outfile_name = f"noisy_dim{noisy_dim}_noise_ratio{noise_ratio}_seed{seed}"
...
def add_header(df):
    ...
    results['noisy_ratio'] = noise_ratio
